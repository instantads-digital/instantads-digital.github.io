...
(function wireGrids() { 
   ...
   * indicatorSelector: selector for the indicator container (e.g., '#serviceIndicators' or 
'.about-indicators') 
  ...
  wireGrid('.case-grid', '.case-indicators', '.case-box'); // optional: add .case-indicators in HTML if 
desired 
})(); 
...
getComputedStyle(document.documentElement).getPropertyValue('--primary-color') || '#22c55e' 
: '#666'; 
...
'#22c55e') : '#666'; 
  ...
  prevBtn.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { 
e.preventDefault(); prevBtn.click(); } }); 
  nextBtn.addEventListener('keydown', (e) => { if (e.key === 'Enter' || e.key === ' ') { 
...
(function ensureAriaHiddenForPopup() { 
  const popup = document.getElementById('popupForm'); 
  if (!popup) return; 
  // When popup is shown, set aria-hidden false; when hidden, true (already handled in popup 
logic) 
